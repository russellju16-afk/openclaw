...
export function registerDiscordSubagentHooks(api: OpenClawPluginApi) {
  const resolveThreadBindingFlags = (accountId?: string) => {
    // Read live config so account + threadBinding edits take effect without restart.
    const liveConfig = api.runtime?.config?.loadConfig?.() ?? api.config;
    const account = resolveDiscordAccount({
      cfg: liveConfig,
      accountId,
    });
    const baseThreadBindings = liveConfig.channels?.discord?.threadBindings;
    const accountThreadBindings =
      liveConfig.channels?.discord?.accounts?.[account.accountId]?.threadBindings;
    return {
      enabled:
        accountThreadBindings?.enabled ??
        baseThreadBindings?.enabled ??
        liveConfig.session?.threadBindings?.enabled ??
        true,
