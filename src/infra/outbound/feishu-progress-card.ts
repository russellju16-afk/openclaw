...
type FeishuCardUpdateFn = (params: FeishuCardUpdateParams) => Promise<unknown>;
...
export function createFeishuLongTaskProgressCard(
...
export function isFeishuProgressCardChannel(channel: string | undefined): boolean {
  const normalized = normalizeMessageChannel(channel);
  return normalized === "feishu" || normalized === "lark";
}

export function hasFeishuCardParam(params: Record<string, unknown>): boolean {
  return isRecord(params.card);
}

export function hasFeishuProgressCardParam(params: Record<string, unknown>): boolean {
  return Boolean(resolveFeishuLongTaskProgressCardPayload(params));
}

export function materializeFeishuProgressCardParams(params: Record<string, unknown>): {
  card: Record<string, unknown>;
  summary: ReturnType<typeof summarizeFeishuLongTaskProgressCard>;
} | null {
  const payload = resolveFeishuLongTaskProgressCardPayload(params);
  if (!payload) {
    return null;
  }
  if (isRecord(params.card)) {
    throw new Error("Feishu message action accepts only one of card or progressCard.");
  }
  const card = createFeishuLongTaskProgressCard(payload);
  params.card = card;
  delete params.progressCard;
  delete params.taskProgressCard;
  delete params.longTaskProgressCard;
  return {
    card,
    summary: summarizeFeishuLongTaskProgressCard(payload),
  };
}

function findRegisteredFeishuChannel(channel: ChannelId) {
  const registry = getActivePluginChannelRegistry();
  return registry?.channels.find((entry) => entry.plugin.id === channel);
}

function readUpdateCardFunction(moduleValue: unknown): FeishuCardUpdateFn | undefined {
  if (!isRecord(moduleValue)) {
    return undefined;
  }
  const direct = moduleValue.updateCardFeishu;
  if (typeof direct === "function") {
    return direct as FeishuCardUpdateFn;
  }
  const defaultExport = moduleValue.default;
  if (isRecord(defaultExport) && typeof defaultExport.updateCardFeishu === "function") {
    return defaultExport.updateCardFeishu as FeishuCardUpdateFn;
  }
  return undefined;
}

export async function tryRunRegisteredFeishuCardUpdate(params: {
  cfg: OpenClawConfig;
  channel: ChannelId;
  action: string;
  ...
  if (params.action !== "edit" || !isFeishuProgressCardChannel(params.channel)) {
    return null;
  }
  const card = isRecord(params.actionParams.card) ? params.actionParams.card : undefined;
  const messageId = readString(params.actionParams.messageId);
  if (!card || !messageId) {
    return null;
  }

  const channelEntry = findRegisteredFeishuChannel(params.channel);
  const rootDir = channelEntry?.rootDir;
  if (!rootDir) {
    return null;
  }
  ...
  try {
    moduleValue = await import(pathToFileURL(modulePath).href);
  } catch (err) {
    throw new Error(
      `Failed to load Feishu card update adapter from ${modulePath}: ${String(err)}`,
      {
        cause: err,
      },
    );
  ...
  const updateCardFeishu = readUpdateCardFunction(moduleValue);
  if (!updateCardFeishu) {
    return null;
  }
