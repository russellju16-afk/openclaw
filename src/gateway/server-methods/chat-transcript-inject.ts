...
import { normalizeAssistantTranscriptText } from "../../config/sessions/transcript.js";
...
export function appendInjectedAssistantMessageToTranscript(params: {
  ...
  const normalizedMessage = normalizeAssistantTranscriptText(params.message);
  if (!normalizedMessage) {
    return { ok: false, error: "empty injected assistant message" };
  }
  ...
  const messageBody: AppendMessageArg & Record<string, unknown> = {
    role: "assistant",
    content: [{ type: "text", text: `${labelPrefix}${normalizedMessage}` }],
    timestamp: now,
