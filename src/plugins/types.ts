...
export type OpenClawPluginApi = {
  ...
  /**
   * Config snapshot captured at plugin register time.
   *
   * This field is a one-shot reference and goes stale after any
   * `writeConfigFile` call, because core replaces the global
   * `runtimeConfigSnapshot` reference rather than mutating it in place
   * (see `src/config/io.ts`). Plugin closures that keep reading this field
   * inside tool/hook handlers will silently see old config.
   *
   * Safe usages:
   * - Register-time decisions (e.g. deciding whether to register a tool set
   *   based on the accounts currently configured at load time).
   *
   * @deprecated For runtime/tool-execution paths. Use
   * `api.runtime.config.loadConfig()` instead to get the current live config.
   * The field itself is retained for register-time reads and third-party
   * plugin compatibility; it will not be removed without a major-version
   * change.
   */
  config: OpenClawConfig;
...
export type PluginHookBeforeAgentReplyEvent = {
  ...
  cleanedBody: string;
  /**
   * Original inbound body before command/directive cleanup.
   * Prefer this or senderId over parsing identity back out of sessionKey.
   */
  rawBody?: string;
  /**
   * Original sender identity from the inbound channel context.
   * Session keys are normalized for routing and must not be treated as the
   * authoritative casing-preserving sender id.
   */
  senderId?: string;
  /** Human-readable sender label when available. */
  senderName?: string;
  /** Originating channel/provider for the inbound message. */
  channel?: string;
  /** Originating account id for multi-account channel plugins. */
  accountId?: string;
};
