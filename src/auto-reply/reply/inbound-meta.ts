...
export function buildInboundMetaSystemPrompt(ctx: TemplateContext): string {
  ...
  //
  // Stable sender identity metadata is the exception: some channels expose multiple app-specific
  // sender ids for the same human, and the model needs the canonical identity to reason correctly
  // about approvals, aliases, and session continuity.
  ...
  const payload = {
    ...
    sender_identity:
      safeTrim(ctx.SenderStableId) ||
      safeTrim(ctx.SenderPreferredTarget) ||
      safeStringArray(ctx.SenderAltIds)
        ? {
            stable_id: safeTrim(ctx.SenderStableId),
            alt_ids: safeStringArray(ctx.SenderAltIds),
            preferred_target: safeTrim(ctx.SenderPreferredTarget),
          }
        : undefined,
