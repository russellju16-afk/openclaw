...
import {
  ...
  modelAuthGetApiKeyForModelMock,
...
describe("compactEmbeddedPiSessionDirect hooks", () => {
  ...
  it("refreshes auth and retries compaction once after a session-expired provider error", async () => {
    modelAuthGetApiKeyForModelMock
      .mockResolvedValueOnce({
        apiKey: "stale-token",
        mode: "oauth",
        profileId: "openai-codex-primary",
        source: "profile:openai-codex-primary",
      })
      .mockResolvedValueOnce({
        apiKey: "fresh-token",
        mode: "oauth",
    ...
    sessionCompactImpl
      .mockRejectedValueOnce(
        new Error(
          "unexpected status 401 Unauthorized: Your ChatGPT session expired before this request finished.",
        ),
      )
      .mockResolvedValueOnce({
        summary: "summary",
    ...
    expect(modelAuthGetApiKeyForModelMock).toHaveBeenCalledTimes(2);
    expect(modelAuthGetApiKeyForModelMock).toHaveBeenNthCalledWith(
      2,
