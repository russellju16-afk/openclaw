...
export async function runEmbeddedAttempt(
  ...
  // Setup-phase watchdog: the in-loop run abort timer (scheduleAbortTimer) is
  // only armed AFTER session manager / createAgentSession / auth / resource
  // loader complete — if any of those hang on a silent network stall, nothing
  // aborts the run for up to params.timeoutMs of silence (historically 48h
  // default). This watchdog fires runAbortController.abort once the configured
  // timeout elapses regardless of which setup step is in flight, so fetch-based
  // hangs (undici via signal) surface as an error instead of indefinite silence.
  let setupWatchdogTimer: NodeJS.Timeout | null = null;
  const clearSetupWatchdog = () => {
    if (setupWatchdogTimer) {
      clearTimeout(setupWatchdogTimer);
  ...
  if (Number.isFinite(params.timeoutMs) && params.timeoutMs > 0) {
    setupWatchdogTimer = setTimeout(
      () => {
        setupWatchdogTimer = null;
        log.warn(
          `embedded run setup-phase timeout: runId=${params.runId} sessionId=${params.sessionId} timeoutMs=${params.timeoutMs}`,
        );
        runAbortController.abort(
          new Error(
            `embedded run setup timeout after ${params.timeoutMs}ms (runId=${params.runId} sessionId=${params.sessionId})`,
          ),
        );
      },
  ...
  try {
    ...
    try {
      ...
      // Setup phase finished; hand off from setup-phase watchdog to the
      // compaction-aware run timer.
      clearSetupWatchdog();
