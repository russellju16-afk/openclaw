...
function shouldFailClosedInterpreterPreflight(command: string): {
  ...
  const hasInterpreterAndScriptHintInSameSegment = (rawText: string): boolean => {
    const segments = splitShellSegmentsOutsideQuotes(rawText, {
      splitPipes: true,
    });
  ...
  const hasInterpreterPipelineScriptHintInSameSegment = (rawText: string): boolean => {
    const commandSegments = splitShellSegmentsOutsideQuotes(rawText, {
      splitPipes: false,
    });
    return commandSegments.some((segment) => {
      const pipelineCommands = splitShellSegmentsOutsideQuotes(segment, {
        splitPipes: true,
      });
...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      // Stamp the caller identity into the child env so downstream tools
      // can cross-check `--caller` and refuse spoofed local invocations.
      applyAgentContextEnv(env, defaults?.senderId);
