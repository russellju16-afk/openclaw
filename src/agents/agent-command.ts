  ...
  try {
    ...
    try {
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        runId,
        agentDir,
        backoffScopeKey: sessionKey ?? sessionId,
    ...
    const payloads = result.payloads ?? [];
    if (sessionFile) {
      const desiredTranscriptText = payloads
        .map((payload) => payload.text?.trim() ?? "")
        .filter(Boolean)
        .join("\n")
        .trim();
      try {
        await sanitizeLatestAssistantTranscriptMessageInFile({
          sessionFile,
          sessionKey,
          desiredText: desiredTranscriptText || null,
        });
      } catch (error) {
        log.warn(
          `assistant transcript sanitize failed for ${sessionKey ?? sessionId}: ${
